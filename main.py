...
def generate_visual_map(output_path, ikari_stores, competitors):
    ...
    for name, (lat, lon) in ikari_stores.items():
        ...
        # ブライトグリーン丸
        draw.ellipse([px-r, py-r, px+r, py+r], fill=(16, 185, 129))
        # テキストラベル背景
        text_w = draw.textlength(name, font=font_bold)
        draw.rectangle([px - text_w/2 - 4, py - r - 20, px + text_w/2 + 4, py - r - 3], fill=(255, 255, 255), outline=(16, 185, 129), width=1)
        # ラベル描画
        draw.text((px, py - r - 19), name, fill=(16, 185, 129), font=font_bold, anchor="ma")
